...
struct TreeIterator
{
  ...
  operator* () const
  {
    return this->m_nodePtr->data; // Return the data associated with the referenced node.
  }
...
private:
  ...
  increment (ConstNodePtr n)
  {
    ...
    //Should it increment if n->parent is still equal to n?
...
public:
  ...
  void
  helper(typename iterator::NodePtr r){
    if(r == nullptr){
      return;
    }
    insert(r->data);
  ...
  begin ()
  {
    ...
    return iterator(minimum(m_header.parent));
  }
  ...
  begin () const
  {
    return const_iterator(minimum(m_header.parent));
  }
...
private:
  ...
  {
    ConstNodePtr current = m_header.parent;
    if(current == nullptr){
      return nullptr;
    }
  ...
  insert (const T& v, NodePtr& r, NodePtr parent)
  {
    ...
    // Use "parent" for recursion and setting the parent of the_a)
    //   node containing "v".
  }
